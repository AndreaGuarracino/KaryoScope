...
A contig's chromosome is decided by the annotated base pairs each chromosome label covers on it, read from the unbinned chromosome annotation (streamed; memory is O(sequences × labels)). Earlier versions voted on the widths of the winner-take-all 1 Mb bins, which is only a proxy for coverage: a bin won by a hair cast a full-width vote and a trailing runt bin was folded into its neighbour's row, so a genuinely mixed contig (e.g. a 2.5 Mb pseudo-autosomal / X-transposed contig) could be assigned to the wrong sex chromosome. The bin-width vote remains as a fallback when only the binned view is available. Over 972 HPRC contigs 2.3 % change assignment, almost all small acrocentric contigs swapping among acrocentrics.
...
| `--label-grammar [plain\|cytoband]` | How to read chromosome identity and arm out of a feature label. `plain` (default, the previous behaviour) takes the chromosome set's label as the chromosome and derives the arm from the region label's category. `cytoband` is **experimental**: it parses both off a cytoband label (`Yq12` → chrY, q arm), so satellite-rich arms such as Yq12 still count as arm material for the orientation vote (the region set routes every satellite class into its centromere catch-all, leaving chrY ~35 % arm-labelled vs ~98 % under cytoband). Not yet validated against a verified flip list, and cytoband resolves acrocentric short arms *worse* than the region set (30–48 % arm-labelled), so flips there may get worse; the likely end state is per-chromosome grammar selection. Requires `--db` to point at a cytoband database. [default: plain] |
