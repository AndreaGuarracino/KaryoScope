...
def test_empty_input_yields_an_empty_sidecar_not_a_failure(tmp_path: Path) -> None:
    """grep exits 1 on no match; the guard makes zero records empty output."""
...
_TAIL = "cut -f1 | cut -d ' ' -f1 | cut -c2- | bgzip > n.gz"


def test_scan_reads_a_plain_file_with_the_selector_alone(plain_bgzip) -> None:
    """No parser, no cat: grep/sed open the file themselves."""
    assert _scan_pipeline(Path("asm.fa"), Path("n.gz")) == (
        f"(grep '^>' asm.fa || [ $? -eq 1 ]) | {_TAIL}"
    )
    assert _scan_pipeline(Path("r.fastq"), Path("n.gz")) == f"sed -n 'p;n;n;n' r.fastq | {_TAIL}"


def test_scan_decompresses_gz_in_front_of_the_selector(plain_bgzip) -> None:
    assert _scan_pipeline(Path("r.fq.gz"), Path("n.gz")) == (
        f"gzip -dc r.fq.gz | sed -n 'p;n;n;n' | {_TAIL}"
    )
    assert _scan_pipeline(Path("asm.fasta.gz"), Path("n.gz")) == (
        f"gzip -dc asm.fasta.gz | (grep '^>' || [ $? -eq 1 ]) | {_TAIL}"
    )


def test_scan_passes_threads_to_bgzip(plain_bgzip) -> None:
    assert _scan_pipeline(Path("r.fq"), Path("n.gz"), threads=8).endswith("| bgzip -@ 8 > n.gz")


def test_scan_quotes_paths(plain_bgzip) -> None:
...
def test_sink_selects_fasta_headers_then_bgzips(plain_bgzip, tmp_path: Path) -> None:
    ...
    sink = names_sink(sidecar, threads=3)
    assert sink.startswith("(grep '^>' || [ $? -eq 1 ]) | cut -f1")
    assert sink.endswith(f"| bgzip -@ 3 > '{sidecar}'")
...
def test_write_runs_one_linear_pipefail_pipeline(
    ...
    pipeline = cmd[4]
    assert pipeline.startswith("gzip -dc reads.fq.gz | sed -n 'p;n;n;n' | cut")
...
def test_write_uses_samtools_for_an_alignment(
    ...
    assert captured["cmd"][4].startswith("/bin/samtools fasta -F 0x900 -N -@ 2 aln.bam | (grep")
